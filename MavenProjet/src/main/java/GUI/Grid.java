...
public class Grid {
	...
	public boolean hasFixedNeighbour(Piece p) {
		...
		return bool;*/
		if(this.listOfFixedNeighbours(p).size()==4) return true;
		return false;
	}
	...
	public boolean hasAtLeast1FixedNeighbour(Piece p) {
		if(this.listOfFixedNeighbours(p)!=null) return true;
		return false;
	}
	...
	public ArrayList<Piece> listOfNeighbours(Piece p) {
		...
		if(p.getPosX()!=this.width-1) {
			lp.add(this.getAllPieces()[p.getPosY()][p.getPosX()+1]);
		}
		if(p.getPosY()!=this.height-1) {
			lp.add(this.getAllPieces()[p.getPosY()+1][p.getPosX()]);
		}
		if(p.getPosX()!=0) {
			lp.add(this.getAllPieces()[p.getPosY()][p.getPosX()-1]);
		}
	...
	/**
	 * list of fixed neighbors
	 * 
	 * @param p
	 *            the piece
	 * @return the list of neighbors
	 */
	public ArrayList<Piece> listOfFixedNeighbours(Piece p) {
		ArrayList<Piece> lp = new ArrayList<Piece>();
		for (Piece p_tmp : this.listOfNeighbours(p)) {
			if (p_tmp.isFixed()) {
				lp.add(p_tmp);
			}
		}
	...
	public boolean arePiecesConnected(Piece p1,Piece p2)
	{
			boolean arePiecesConnected=false;
			if (p1.hasRightConnector() && p2.hasLeftConnector() && p1.getPosY()+1==p2.getPosY()) arePiecesConnected = true;
			else if (p1.hasLeftConnector() && p2.hasRightConnector() && p1.getPosY()-1==p2.getPosY()) arePiecesConnected = true;
			else if (p1.hasBottomConnector() && p2.hasTopConnector() && p1.getPosX()-1==p2.getPosX()) arePiecesConnected = true;
			else if (p1.hasTopConnector() && p2.hasBottomConnector() && p1.getPosX()+1==p2.getPosX()) arePiecesConnected = true;
			return arePiecesConnected;
	}
	
	public boolean isFilled(Piece p,ArrayList<Piece> piecePossible){
		boolean isFilled=false;
		for (Piece p_tmp:this.listOfNeighbours(p)){
			if(p_tmp.getType()!=PieceType.VOID && !piecePossible.contains(p_tmp)) {
				isFilled=true;
			}
		}
		return isFilled;
